          ...
          <form>
            ...
            <div class="text-center">
              <small class="text-muted text-center">
                Remember your password?<nuxt-link to="/hspace/signin">Sign in</nuxt-link>.
              </small>
            </div>
          </form>
        </div>
...
export default {
  data () {
    return {
      ...
      errorCode: false,
      input: {
        token: this.$store.state.AdmintokenData,
        newPassword: ''
        // confirmPassword: ''
      },
      confirmPassword: '',
  ...
  methods: {
    async validate () {
      if (this.input.newPassword === this.confirmPassword) {
        this.isLoading = true
        const res = await apiService.request(false, urls.RESETPASSWORDSUPERADMIN, this.input, 'POST', 'SUPERADMIN_TOKEN')
